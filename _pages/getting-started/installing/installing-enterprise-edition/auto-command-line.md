...
2.  Create a configuration file and save as `genconf/config.yaml`. For parameters descriptions and configuration examples, see the [documentation][1].
    ...
    **Important:** You cannot use an NFS mount for Exhibitor storage (`exhibitor_storage_backend: shared_filesystem`) with the automated command line installation method. To use an NFS mount for Exhibitor storage, you must use the [Manual command line installation method][2].
